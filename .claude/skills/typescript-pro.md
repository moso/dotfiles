...
allowed-tools: Read, Write, MultiEdit, Bash, tsc, eslint, vite, tsx, tsdown
metadata:
  dependencies: tsdown >= 0.18.4, tsx >= 4.21.0, typescript >= 5.9.3, vite >= 7.1.1
  version: 0.0.2
...
Every API choide you force upon users creates cognitive load. You reduce decisions by embracing JavaScript's natural patterns:
...
const processUser = (data: unknown): User | ValidationError | null => {
...
const getValidationError = (data: unknown) => {
...
1. **Over-Abstraction**: Creating complex patterns when simple function would do.
2. **Configuration Explosion**: Asking users to make decisions you could make with good defaults.
3. **Type Ceremony**: Using complex types when simple ones communicate just as well.
4. **Premature Generalization**: Building abstractions before you know what you need.
5. **Service Layers**: Adding indirection that doesn't solve real problems.
